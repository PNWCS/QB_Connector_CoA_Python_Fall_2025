...
This project handles the Chart of Accounts synchornisation between an excel workbook and Quickbooks. The connector takes in a .xlsx workbook, and adds the accounts from the workbook to quickbooks.
...
2. Build the executable:
    ```bash
    poetry run pyinstaller --onefile --name Chart_of_accounts --hidden-import win32timezone --hidden-import win32com.client build_exe.py
    ```
...
2. Run the executable with the same arguments the Python entry point expects:
   ```cmd
   ./Chart_of_accounts.exe --workbook C:\path\to\ChartOfAccounts.xlsx --output C:\path\to\report.json
   ```
...
```cmd
.\Chart_of_accounts.exe --workbook C:\Users\\MunozJ\QB_Connector_CoA_Python_Fall_2025\company_data.xlsx --output C:\Users\MunozJ\QB_Connector_CoA_Python_Fall_2025\report.json
```
